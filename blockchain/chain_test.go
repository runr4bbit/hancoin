...
func TestBlocks(t *testing.T) {
	...
	if reflect.TypeOf(blocksResult) != reflect.TypeOf([]*Block{}) {
		t.Error("Block() should return a slice of blocks.")
	}
}
...
func TestGetDifficulty(t *testing.T) {
	...
	tests := []test{
		{heignt: 0, want: defaultDifficulty},
		{heignt: 2, want: defaultDifficulty},
		{heignt: 5, want: 3},
	}
	for _, tc := range tests {
		bc := &blockchain{Height: tc.heignt, CurrentDifficulty: defaultDifficulty}
